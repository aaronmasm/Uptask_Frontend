...
export default function ProjectDetailsView() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["project", proyectId],
    queryFn: () => getFullProject(proyectId),
    retry: false,
  });
        ...
        {isManager(data.manager, user._id) && (
          <nav className="my-5 flex gap-3">
            ...
            <Link
             ...
             bg-fuchsia-600 hover:bg-fuchsia-700 px-10 py-3 text-white
             text-xl font-bold cursor-pointer transition-colors text-center"
            >
              Colaboradores
            </Link>
          </nav>
